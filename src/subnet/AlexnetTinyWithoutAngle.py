...
class AlexnetTinyWithoutAngle(SubnetBase):
	...
	def buildNetVariables(self):
		weights = {
			...
			'fcW1'  : tf.Variable(tf.random_normal([4*4*16, 128], stddev=0.01)),
	...
	def buildNetBody(self, weights, biases):
		...
		pool4reshape = tf.reshape(norm4, [-1, weights['fcW1'].get_shape().as_list()[0]])

		fc1 = tf.add(tf.matmul(pool4reshape, weights['fcW1']), biases['fcb1'])
		fc1 = tf.nn.relu(fc1)
